...
def main():
    ...
    post_proc_stream = torch.cuda.Stream()
    ...
    def loading_one_data(img_path, idx, imgs, load_stream):
        with NVTXUtil(f"load image{idx}", "red", mm), torch.cuda.stream(load_stream):
            if isinstance(img_path, str):
                img_name = os.path.basename(img_path)
                basename, ext = os.path.splitext(img_name)
                print(f'[{idx}/{test_img_num}] Processing: {img_name}')
                img = cv2.imread(img_path, cv2.IMREAD_COLOR)
            else: # for video processing
    ...
    thread_imgs = {}
    thread_prev = threading.Thread( target=loading_one_data, args=(img_path, 0, thread_imgs, load_stream) )
    thread_prev.start()
    ...
    for i in range(len(input_img_list)):
        ...
        if i<(len(input_img_list)-1):
            thread_imgs = {}
            thread_cur = threading.Thread( target=loading_one_data, args=(input_img_list[i+1], i+1, thread_imgs, load_stream) )
            thread_cur.start()
        ...
        with NVTXUtil(f"img{i}", "red", mm), SynchronizeUtil(torchutil.torch_stream):
            ...
            outputs = []
            for idx, cropped_face in enumerate(face_helper.cropped_faces):
                with NVTXUtil(f"for cropped face{idx}", "red", mm):
                    ## prepare data
                    ...
                    try:
                        with NVTXUtil(f"step1", "red", mm):
                            with torch.no_grad():
                                #if False:
                                if net_trt_engine:
                                    with NVTXUtil(f"trt{i}", "red", mm):
                                        print(f"using net_trt_engine")
                                        trt_output = net_trt_engine.inference(inputs=[cropped_face_t.to(torch.float32).contiguous()],
                                                            outputs = net_trt_engine.output_tensors)
                                        output = trt_output[0].to(torch.float32)
                                else:
                                    with NVTXUtil(f"torch{i}", "red", mm):
                                        print(f"using torch")
                                        outputs = net(cropped_face_t, w=w, adain=True)
                                        for out in outputs:
                                            print(f"out: {out.shape, out.dtype, out.device}")
                                        output = outputs[0]
                                print(f"output: {output.shape, output.dtype, output.device}")
                                cropped_face_t = output
                    except Exception as error:
                        print(f'\tFailed inference for CodeFormer: {error}')
                    outputs.append(cropped_face_t)

        def post_processing_img(idx, face_helper_, outputs_, basename_, img_, post_proc_stream):
            with NVTXUtil(f"post_processing_img{idx}", "red", mm), torch.cuda.stream(post_proc_stream):
                for idx, cropped_face in enumerate(face_helper_.cropped_faces):
                    restored_face = tensor2img(outputs_[idx], rgb2bgr=True, min_max=(-1, 1))
                    restored_face = restored_face.astype('uint8')
                    with NVTXUtil(f"add_restored_face", "red", mm):
                        face_helper_.add_restored_face(restored_face, cropped_face)
                ...
                if not args.has_aligned:
                    ...
                    if bg_upsampler is not None:
                        ...
                        bg_img = None
                    face_helper_.get_inverse_affine(None)
                    # paste each restored face to the input image
                    if args.face_upsample and face_upsampler is not None: 
                        restored_img = face_helper_.paste_faces_to_input_image(upsample_img=bg_img, draw_box=args.draw_box, face_upsampler=face_upsampler)
                    else:
                        restored_img = face_helper_.paste_faces_to_input_image(upsample_img=bg_img, draw_box=args.draw_box)

                # save faces
                for idx, (cropped_face, restored_face) in enumerate(zip(face_helper_.cropped_faces, face_helper_.restored_faces)):
                    with NVTXUtil(f"save face", "red", mm):
                        # save cropped face
                        if not args.has_aligned: 
                            save_crop_path = os.path.join(result_root, 'cropped_faces', f'{basename_}_{idx:02d}.png')
                            imwrite(cropped_face, save_crop_path)
                        # save restored face
                        if args.has_aligned:
                            save_face_name = f'{basename_}.png'
                        else:
                            save_face_name = f'{basename_}_{idx:02d}.png'
                        if args.suffix is not None:
                            save_face_name = f'{save_face_name[:-4]}_{args.suffix}.png'
                        save_restore_path = os.path.join(result_root, 'restored_faces', save_face_name)
                        imwrite(restored_face, save_restore_path)
                ...
                # save restored img
                if not args.has_aligned and restored_img is not None:
                    with NVTXUtil(f"imwrite", "red", mm):
                        if args.suffix is not None:
                            basename_ = f'{basename_}_{args.suffix}'
                        save_restore_path = os.path.join(result_root, 'final_results', f'{basename_}.png')
                        imwrite(restored_img, save_restore_path)

        thread_post_processing_img = threading.Thread( target=post_processing_img, args=(i, copy(face_helper), copy(outputs), copy(basename), prev_imgs[0], post_proc_stream) )
        thread_post_processing_img.start()

        thread_prev = thread_cur

    thread_post_processing_img.join()
