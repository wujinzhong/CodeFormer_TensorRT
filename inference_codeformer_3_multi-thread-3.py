...
def main():
    ...
    for i, img_path in enumerate(input_img_list):
                    ...
                    try:
                        with NVTXUtil(f"step1", "red", mm):
                            with torch.no_grad():
                                #if False:
                                if net_trt_engine:
                                    with NVTXUtil(f"trt", "red", mm):
                                        print(f"using net_trt_engine")
                                        trt_output = net_trt_engine.inference(inputs=[cropped_face_t.to(torch.float32).contiguous()],
                                                            outputs = net_trt_engine.output_tensors)
                                        output = trt_output[0].to(torch.float32)
                                else:
                                    with NVTXUtil(f"torch", "red", mm):
                                        print(f"using torch")
                                        outputs = net(cropped_face_t, w=w, adain=True)
                                        for out in outputs:
                                            print(f"out: {out.shape, out.dtype, out.device}")
        ...
        def post_processing_img(face_helper_, outputs_, basename_, img_, post_proc_stream):
            with NVTXUtil(f"post_processing_img", "red", mm), torch.cuda.stream(post_proc_stream):
                ...
                for idx, (cropped_face, restored_face) in enumerate(zip(face_helper_.cropped_faces, face_helper_.restored_faces)):
                    with NVTXUtil(f"save face{idx}", "red", mm):
                        # save cropped face
                        if not args.has_aligned: 
        ...
        thread_post_processing_img = threading.Thread( target=post_processing_img, args=(copy(face_helper), copy(outputs), basename, prev_imgs[0], post_proc_stream) )
